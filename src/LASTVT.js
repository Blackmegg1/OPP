...
export default function getLASTVT(grammarOBJ) {
    ...
    while (stack.length !== 0) {
        ...
        ruleTwo(topElement, LASTVT, findVNindex, findVTindex, stack, unfoldGrammarArr);
    }
...
function ruleOne(g, LASTVT, findVNindex, findVTindex, stack) { //规则一：形如P->…a 或P->…aQ 则a属于P
    const gArr = g.split("->");
    const left = findVTindex(gArr[0]); //找到的P索引
    let right = -1; //开始找a的索引
    const len = gArr[1].length; //生成式右部的长度
    if (tools.isValid(gArr[1][len - 1])) { //形如 P->…a 
        right = findVNindex(gArr[1][len - 1]);
    }
    else if (len > 1 && tools.isValid(gArr[1][len - 2])) { //P->…aQ
        right = findVNindex(gArr[1][len - 2]);
    }
...
function ruleTwo(g, LASTVT, findVNindex, findVTindex, stack, unfoldGrammarArr) { //规则二：形如 a属于Q , 若P->……Q , 则a属于P
    ...
    for (const gra of unfoldGrammarArr) {
        ...
        if (findVTindex(gra[len-1]) === Q && gra[0] !== gra[len-1]) { //找到形如 P->……Q 且 P!=Q防止产生循环
            P = findVTindex(gra[0]);
            LASTVT[P][a] = true;
